...
def setup_monitor(
    ...
    request.cls.online_devices = [
        request.cls.mock_device, request.cls.mock_device
    ]

    request.cls.mock_ssh_client = mocker.Mock(spec=SSHClient)
    request.cls.mock_logging = mocker.patch("src.monitor.logging.info")

    request.cls.mock_calc_elapsed_time = mocker.patch(
        "src.monitor.calc_elapsed_time", return_value=CHECK_INTERVAL
    )
...
class TestMonitor:
    def test_monitor_successful_run(self):
        ...
        self.mock_logging.assert_called_with(
            f"{len(self.online_devices)} devices online"
        )

    def test_monitor_device_offline(self, mocker: pytest_mock.MockerFixture):
        self.mock_device.check_status = mocker.Mock(
            side_effect=DeviceOffline("Device offline")
        )

        with pytest.raises(DeviceOffline, match="Device offline"):
