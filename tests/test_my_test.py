...
def setup_my_test(
    ...
    request.cls.mock_create_connection = mocker.patch(
        'my_test.create_connection'
    )

    request.cls.mock_read_remote_file = mocker.patch(
        'my_test.read_remote_file'
    )
...
@pytest.mark.test_main
@pytest.mark.usefixtures("setup_my_test")
class TestMain:
    def test_main_success(self):
        self.mock_create_connection.return_value = (
            self.mock_client_A, self.mock_client_B, self.mock_sftp_client
        )

        self.mock_read_remote_file.return_value = (
        ...
        self.mock_read_remote_file.assert_called_once_with(
            self.mock_sftp_client,
            CSV_REMOTE_PATH
        )
    ...
    def test_main_no_online_devices(self):
        self.mock_create_connection.return_value = (
            self.mock_client_A, self.mock_client_B,
            self.mock_sftp_client
        )
        ...
        self.mock_read_remote_file.return_value = (
            ...
            "vendor,model,firmware_version,status\n"
            "1,router-alpha,192.168.1.1,"
            "CircuitWizard,Model4331,16.9.2,offline"
        )
    ...
    def test_main_exception_handling(self):
        self.mock_create_connection.side_effect = ConnectError(
            "Connection error"
        )
